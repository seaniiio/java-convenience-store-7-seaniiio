...
- [x] 주문 목록을 검증 후 저장한다.
  - [x] 재고가 충분한지 확인한다.
- [x] 주문에 대해 프로모션 적용 여부를 확인한다.
  - [x] 오늘 날짜를 기반으로 프로모션이 적용되는지 확인한다.
  - [x] 프로모션이 적용되는 경우 
    - [x] 프로모션 재고를 먼저 차감한다.
    - [x] 프로모션 재고가 부족한 경우 일반 재고를 차감한다.
    - [x] 프로모션 재고가 부족한 경우를 확인한다.
    - [x] 프로모션 조건만큼 구매하지 않았는지 확인한다.
- [x] 멤버십 할인을 적용하는 경우
  - [x] 프로모션 미적용 금액의 30%를 할인한다.
  - [x] 최대 8,000원을 적용한다.
- [x] 영수증 출력에 필요한 값을 계산한다.

### 예외 상황
- [x] 구매할 상품과 수량 형식이 올바르지 않은 경우
- [x] 존재하지 않는 상품을 입력한 경우
- [x] 구매 수량이 재고 수량을 초과한 경우
- [x] 의사를 묻는 질문에 Y/N 외의 입력이 주어진 경우
- [x] 중복된 상품을 구매하려 하는 경우
